    ...
    try {
      ...
      if (response.status === 429) {
        if (retryCount < MAX_RETRIES) {
          ...
          return runJudge0(language, code, stdin, retryCount + 1);
        }
      ...
      const executionTime = data.time ? Math.round(parseFloat(data.time) * 1000) : Math.round(performance.now() - startTime);
      ...
      // Compilation Error
      if (statusId === 6) {
        return { success: false, output: "", error: compileOutput || "Compilation failed", executionTime, memory };
      }

      // Accepted
      if (statusId === 3) {
        return { success: true, output: stdout.trim(), executionTime, memory };
      }

      // TLE
      if (statusId === 5) {
        return { success: false, output: stdout.trim(), error: "Time Limit Exceeded", executionTime, memory };
      }

      // Runtime Error
      if (statusId >= 11) {
        const errorMsg = stderr || data.status?.description || "Runtime error";
        return { success: false, output: stdout.trim(), error: errorMsg, executionTime, memory };
      }

      // Other
      return { success: false, output: stdout.trim(), error: stderr || data.status?.description || "Execution failed", executionTime, memory };
    } catch (e: any) {
      if (retryCount < MAX_RETRIES) {
        const delay = BASE_DELAY_MS * Math.pow(2, retryCount);
        await new Promise(r => setTimeout(r, delay));
  ...
  const executeWithJudging = useCallback(async (
    ...
    try {
      ...
      for (let i = 0; i < testCases.length; i++) {
        ...
        // Execute code
        const result = await runJudge0(language, codeToRun, "");
