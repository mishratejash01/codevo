...
export function Header({ session, onLogout }: HeaderProps) {
  ...
  return (
    <header className={cn(
      "fixed z-[100] left-0 right-0 mx-auto transition-all duration-700 font-sans",
      isScrolled ? "top-6 max-w-7xl px-4 md:px-0" : "top-0 w-full max-w-full px-10 py-6"
    )}>
      {/* Background layer with blur - z-[-1] to stay behind dropdowns */}
      <div className={cn(
        "absolute inset-0 transition-all duration-700 z-[-1]",
        isScrolled ? "rounded-2xl border border-white/10 bg-black/60 backdrop-blur-xl shadow-2xl" : "bg-transparent border-transparent"
      )} />

      <nav className="flex items-center justify-between w-full relative px-10 h-11">
        {/* Logo Section */}
        ...
        <div className="hidden md:flex flex-1 justify-end items-center gap-9 mr-8">
          ...
          >
            ...
            {activeDropdown === 'products' && (
              <div className="absolute top-full right-[-180px] w-[950px] bg-[#050505] border border-white/10 rounded-sm p-10 grid grid-cols-[1fr_1fr_1.2fr] gap-10 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.9)] backdrop-blur-3xl animate-in fade-in slide-in-from-top-2 duration-300 z-[1000]">
            ...
            <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
              ...
              <PopoverContent align="end" className="w-[320px] p-8 bg-[#0a0a0a] border border-[#222222] shadow-[0_40px_80px_rgba(0,0,0,0.9)] outline-none ring-0">
                <div className="text-center">
                  ...
                  <Link to="/profile" className="w-full block py-4 text-[13px] font-bold uppercase tracking-widest bg-white text-black hover:bg-zinc-200 transition-all rounded-sm border-none shadow-none">
                    Edit Profile
                  </Link>
                  <div className="mt-4 pt-4 border-t border-[#222222]">
                    <button onClick={onLogout} className="flex items-center justify-center w-full text-white text-[13px] font-medium hover:text-zinc-400 transition-colors py-2 border-none outline-none focus:ring-0">
